...
export const GUIDE_SECTIONS: { title: string; body: ReactNode }[] = [
  {
    ...
    body: (
      <p>
        欢迎来到 <b>GG RESET</b>，在这里你可以通过计时、计数和呼吸调整来帮助你稳固信念。
      </p>
    ),
  },
  {
    ...
    body: (
      <>
        <p className="font-medium">1. 设置主题和肯定语</p>
        <p className="mt-1">
          进入【显化列表】页面您可以增加或删除主题标签（在专注页面和数据中心会有用！），在主题标签下添加肯定语。在下方的显化列表里增加目标，可拖动调整顺序。
        </p>
        <p className="font-medium mt-3">2. 进入专注</p>
        <p className="mt-1">
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p className="mt-1">
          计数器和计时器的功能可以单独使用。您可以选择手动计数肯定语不选择计时，也可以选择只开启主题计时来记录日常工作学习和运动的时长数据。选择正计时自动计数功能请记得在结束的时候返回 gg reset 关闭计时计数。
        </p>
        <p className="mt-1">
          【网站更新】重新加载网页以更新至最新版本。一些时候系统可能会突然卡回旧的版本或有偶发性故障，重刷新页面就会恢复正常更新到最新的版本。刷新和重新输入网址并不会影响数据。
        </p>
        <p className="mt-1">
          【肯定语】可在设置里选择计数器显示"今日计数"或"累积计数"，开启计数器"重置"开关（开启后在计数器下方会有一个重置按钮就，用户可以选择清空今日计数或累计计数）。在「显化列表」页面可手动输入肯定语数值。
        </p>
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          所有数据均存储于浏览器本地，暂不支持云端共享功能。请不要在无痕浏览页使用 gg reset，或清除浏览器缓存或数据，这些都会导致 gg reset 数据丢失。
        </p>
        <p className="mt-2">
